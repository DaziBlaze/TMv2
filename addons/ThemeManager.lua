...
if typeof(copyfunction) == "function" then
    ...
    local success, err = pcall(function() return isfolder_copy("test"..tostring(math.random(1000000,9999999))) end)
    if not success or typeof(err) ~= "boolean" then
        isfolder = function(folder) local ok, data = pcall(isfolder_copy, folder) return ok and data or false end
        isfile = function(file) local ok, data = pcall(isfile_copy, file) return ok and data or false end
        listfiles = function(folder) local ok, data = pcall(listfiles_copy, folder) return ok and data or {} end
    end
end
...
do
    ...
    ThemeManager.BuiltInThemes = {
        ...
        ["BBot"] = {2, httpService:JSONDecode([[{"FontColor":"ffffff","MainColor":"1e1e1e","AccentColor":"#ff0000","BackgroundColor":"232323","OutlineColor":"141414"}]])},
    ...
    function ThemeManager:ApplyTheme(theme)
        ...
        for idx,val in pairs(customThemeData or scheme) do
            if idx=="VideoLink" then continue
    ...
    function ThemeManager:CreateThemeManager(groupbox)
        ...
        groupbox:AddDropdown("FontFace",{Text="Font Face",Default="Code",Values={"BuilderSans","Code","Fantasy","Gotham","Jura","Roboto","RobotoMono","SourceSans"}})

        local ThemesArray = {}
        for Name,Theme in pairs(self.BuiltInThemes) do table.insert(ThemesArray,Name) end
        ...
        groupbox:AddButton("Delete theme",function()
            ...
            if not success then return self.Library:Notify("Failed to delete theme: "..err) end
        ...
        groupbox:AddButton("Reset default",function()
            ...
            if not success then return self.Library:Notify("Failed to reset default: delete file error") end
